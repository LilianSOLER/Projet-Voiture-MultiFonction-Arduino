#define pot 5
#define LR 4
#define LO 3
#define LV 2
int tension_num_5 = 0 ;
float tension_5 = 0 ;

//void liretension(int port,int tension_num_port,float tension_port){
 // tension_num_port = analogRead(port);
 // tension_port = analogRead(port)/204.8;
 // Serial.println("La tension numérique " + String(port) +" est: " + String(tension_num_port));
 // Serial.println("La tension réel " + String(port) + " est: " + String(tension_port) + " Volt" );
 // return tension_port;
 // }

void EteintTout(){
digitalWrite(LV,1);
digitalWrite(LO,1);
digitalWrite(LR,1);
Serial.println("Toutes les Leds sont éteintes");
}


void Rituel(){
  Serial.begin(9600); // Initialisation de la transmission
  EteintTout();
  digitalWrite(LR,0);Serial.println("Led de départ"); //Rituel de départ
  delay(2000);
  digitalWrite(LR,1);Serial.println("Led de départ fin");
}
  

void setup() {
  pinMode(LV,OUTPUT); //Branche le port 2 comme une sortie
  pinMode(LO, OUTPUT); //Branche le port 3 comme une sortie
  pinMode(LR, OUTPUT); //Branche le port 4 comme une sortie
  Rituel();
  }

void loop() {
  //liretension(pot,tension_num_5, tension_5);
  tension_num_5 = analogRead(5);
  tension_5 = analogRead(5)/204.8;
  Serial.println("La tension numérique " + String(5) +" est: " + String(tension_num_5));
  Serial.println("La tension réel " + String(5) + " est: " + String(tension_5) + " Volt" );
  if (tension_5 <= float(3)){Serial.println("Entrée dans le premier if");digitalWrite(LV,0);Serial.println("Led vert allumé");; digitalWrite(LO,1);Serial.println("Led Orange éteinte");digitalWrite(LR,1);Serial.println("Led Rouge éteinte");}
  if (tension_5 > float(4)){Serial.println("Entrée dans le second if");digitalWrite(LV,0);Serial.println("Led vert allumé"); digitalWrite(LO,0);Serial.println("Led Orange allumé");digitalWrite(LR,0);Serial.println("Led Rouge allumé");}
  if ((tension_5 <= float(4))&&(tension_5 > float(3))) {Serial.println("Entrée dans le troisième if");digitalWrite(LV,0);Serial.println("Led vert allumé");digitalWrite(LO,0);Serial.println("Led Orange allumé");;digitalWrite(LR,1);Serial.println("Led Rouge éteinte");}
  
}
